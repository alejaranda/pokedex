...
export function PokemonCard({ p }: Props) {
  ...
  return (
    <article
      className="flex overflow-hidden rounded-lg border border-zinc-200 bg-white transition hover:-translate-y-0.5 hover:border-zinc-300 sm:flex-col"
      style={{
        ["--pk-color" as any]:
          `var(--type-${primary})`,
      }}
      ...
      <div className="flex flex-1 flex-col justify-center p-2.5">
        <div className="mb-1 flex items-baseline justify-between">
          ...
          <span className="font-pokemon text-[6px] text-zinc-500">
            <b className="text-[9px] text-(--pk-color)">
              {p.hp}
            </b>{" "}
            HP
          </span>
