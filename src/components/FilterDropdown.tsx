...
const TYPE_ICONS: Record<string, string> = {
  ...
  poison: `<svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="9" r="4" fill="currentColor"/><path d="M6 5V3h4v2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M5 3h6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>`,
  flying: `<svg viewBox="0 0 16 16" fill="none"><path d="M2 10c2-4 5-5 8-4M14 6c-2 0-4 2-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M2 10c2-1 5-1 7 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>`,
  ...
  fairy: `<svg viewBox="0 0 16 16" fill="none"><path d="M8 2l.8 2.4H14l-2.1 1.6.8 2.4L8 7l-4.7 1.4.8-2.4L2 4.4h5.2L8 2z" fill="currentColor"/><path d="M8 9l.5 1.5H10l-1.3 1 .5 1.5L8 12l-1.2.9.5-1.5L6 10.5h1.5L8 9z" fill="currentColor"/></svg>`,
          ...
          {options.map((option) => (
            <button
              ...
              className={`flex w-full items-center gap-2 px-3 py-2 text-left font-pokemon text-[7px] transition hover:bg-zinc-50
                ${value === option.value ? "font-semibold text-zinc-900" : "text-zinc-500"}`}
            >

              <TypeIcon type={option.value} size={10} />
              {option.label}
              {value === option.value && (
                <svg width="8" height="8" viewBox="0 0 8 8" className="ml-auto text-zinc-900">
                  <path d="M1 4L3 6L7 2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" fill="none" />
                </svg>
              )}
            </button>
          ))}
