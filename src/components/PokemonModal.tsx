...
function computeWeakStrong(types: string[]) {
  ...
  weakSet.forEach((w) => {
    if (strongSet.has(w)) { weakSet.delete(w); strongSet.delete(w); }
  });
        ...
        <div className="flex">
          ...
          >
            ...
            <span
              ...
              style={{ color: `color-mix(in srgb, ${typeColor} 55%, transparent)` }}
            >
              #{num}
            </span>
            <h2
              className="font-pokemon capitalize"
              style={{ fontSize: "11px", color: "var(--pk-ink, #1a1a1a)", margin: 0 }}
            >
              {pokemon.name}
            </h2>
            ...
            <div className="flex gap-3 mt-1">
              <span className="font-pokemon text-[8px]" style={{ color: typeColor }}>
                {pokemon.hp}{" "}
                <span className="text-zinc-400" style={{ fontSize: "6px" }}>hp</span>
              </span>
              <span className="font-pokemon text-[8px]" style={{ color: typeColor }}>
                {(pokemon.height / 10).toFixed(1)}m{" "}
                <span className="text-zinc-400" style={{ fontSize: "6px" }}>ht</span>
              </span>
              <span className="font-pokemon text-[8px]" style={{ color: typeColor }}>
                {ROMAN[pokemon.generation]}
                <span className="text-zinc-400" style={{ fontSize: "6px" }}> gen</span>
              </span>
            </div>
          </div>
          ...
          >
            <div className="p-4 pb-3">
              <p className="mb-2 font-pokemon text-zinc-400" style={{ fontSize: "6px", letterSpacing: "0.1em" }}>
                BASE STATS
              </p>
                ...
                {STAT_CONFIG.map(({ key, label }) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2">
                      <span className="font-pokemon text-zinc-400" style={{ fontSize: "6px", width: "44px" }}>
                        {label}
                      </span>
                      <span className="font-pokemon text-zinc-700" style={{ fontSize: "7px", width: "20px", textAlign: "right" }}>
                        {value}
                      </span>
                      <div
                      ...
                      >
                        <div
                          ...
                          style={{ width: `${pct}%`, height: "5px", background: typeColor }}
                        />
                      </div>
                    </div>
                  );
            ...
            <div style={{ height: "0.5px", background: `color-mix(in srgb, ${typeColor} 15%, transparent)`, margin: "0 16px" }} />

            <div className="p-4 py-3">
              <div className="mb-2">
                <p className="mb-1.5 font-pokemon text-zinc-400" style={{ fontSize: "6px", letterSpacing: "0.1em" }}>
                  weak against
                </p>
                <div className="flex flex-wrap gap-1">
                  {weak.length === 0
                    ? <span className="font-pokemon text-zinc-300" style={{ fontSize: "6px" }}>—</span>
                    : weak.map((t) => (
                      <span
                        key={t}
                        className="font-pokemon capitalize text-white"
            ...
            <div className="p-4 py-3">
              ...
              {evoLoading ? (
                <span className="font-pokemon text-zinc-400" style={{ fontSize: "6px" }}>loading...</span>
              ) : evoChain.length <= 1 ? (
                <span className="font-pokemon text-zinc-300" style={{ fontSize: "6px" }}>no evolution chain</span>
              ) : (
                  ...
                  {evoChain.map((evo, i) => {
                    ...
                    return (
                      <div key={evo.id} className="flex items-center gap-2">
                        {i > 0 && (
                          <span className="font-pokemon text-zinc-400" style={{ fontSize: "7px", flexShrink: 0 }}>→</span>
                        )}
                        <div
